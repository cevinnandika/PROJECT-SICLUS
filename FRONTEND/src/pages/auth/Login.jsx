...
const Login = ({ onLoginSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        if (password === driver.password) {
          setTimeout(() => {
            onLoginSuccess({
              ...
              email: username,
              role: "pengemudi",
              phone: driver.phone || "081234567891",
              trayek: driver.trayek,
              bus: driver.bus
            });
          }, 500);
          return;
      ...
      // Fallback ke API login existing
    ...
    <div className="min-h-screen w-full bg-slate-950 flex items-center justify-center p-4 sm:p-6 lg:p-10 relative overflow-hidden font-sans">
      ...
      <div className={`relative w-full max-w-[360px] xs:max-w-[390px] sm:max-w-[440px] md:max-w-[480px] lg:max-w-[520px] bg-white/85 backdrop-blur-2xl rounded-[2.2rem] sm:rounded-[2.5rem] p-5 sm:p-8 lg:p-10 shadow-[0_25px_60px_-15px_rgba(0,0,0,0.5)] border border-white/70 transition-all duration-1000 ease-out transform ${isMounted ? "translate-y-0 opacity-100" : "translate-y-12 opacity-0"}`}>
        ...
        <div className="flex flex-col items-center text-center mt-1 mb-5 sm:mb-8">
          ...
          <p className="text-[10px] sm:text-xs font-bold text-slate-500 mt-1 tracking-widest leading-relaxed uppercase">
            Sistem Inspeksi & Catatan
            <br />
            Laporan Sopir
          </p>
        </div>
        ...
        <div className="mt-6 pt-6 border-t border-slate-200/60 space-y-3">
          ...
          <div className="grid grid-cols-1 gap-2 text-[10px]">
            <div className="bg-blue-50/80 border border-blue-200 rounded-xl p-2.5">
              <span className="font-bold text-[#00206B] block">Admin:</span>
              <span className="text-slate-600 font-mono block">admin@siclus.id / admin123</span>
            </div>
            <div className="bg-slate-50/80 border border-slate-200 rounded-xl p-2.5">
              <span className="font-bold text-slate-700 block">Supir:</span>
              <span className="text-slate-600 font-mono block">budi@siclus.id / budi123</span>
            </div>
          </div>
        </div>
